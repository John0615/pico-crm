...
pub fn Modal(
    ...
    let modal_box_class = if box_class.contains("max-h-none") && box_class.contains("overflow-visible") {
        // 用户希望禁用滚动限制，使用自定义类
        if box_class.is_empty() {
            "modal-box w-11/12 max-h-none overflow-visible"
        } else {
            format!("{} modal-box w-11/12 max-h-none overflow-visible", box_class)
        }
        ...
        format!("{} modal-box w-11/12 max-h-[80vh] overflow-y-auto", box_class)
    };
