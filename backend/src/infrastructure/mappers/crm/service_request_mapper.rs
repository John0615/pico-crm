...
impl ServiceRequestMapper {
    pub fn to_view(entity: Model) -> SharedServiceRequest {
        SharedServiceRequest {
            ...
            appointment_start_at: entity.appointment_start_at.map(parse_date_time_to_string),
    ...
    pub fn to_domain(entity: Model) -> ServiceRequest {
        let status =
            ServiceRequestStatus::parse(&entity.status).unwrap_or(ServiceRequestStatus::New);
