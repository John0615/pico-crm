...
pub struct SeaOrmAfterSalesCaseRecordQuery {
    db: DatabaseConnection,
    schema_name: String,
}

impl SeaOrmAfterSalesCaseRecordQuery {
...
#[async_trait]
impl AfterSalesCaseQuery for SeaOrmAfterSalesCaseRecordQuery {
    type Result = shared::after_sales::AfterSalesCase;
...
async fn load_operator_names(
    ...
    let users = UserEntity::find()
        ...
        .all(txn)
        .await
        .map_err(|e| format!("query after sales record operators error: {}", e))?;
