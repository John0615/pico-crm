...
#[async_trait]
impl AfterSalesCaseRepository for SeaOrmAfterSalesCaseRecordRepository {
    fn create_case(
        &self,
    ...
    fn update_refund(
        &self,
        _refund: UpdateAfterSalesRefund,
    ) -> impl std::future::Future<Output = Result<AfterSalesCase, String>> + Send {
        async { Err("update_refund is not supported by this repository".to_string()) }
    }
}
