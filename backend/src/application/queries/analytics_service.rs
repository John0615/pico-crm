...
impl AnalyticsQueryService {
    ...
    pub async fn overview(
        ...
        let total_users =
            self.count_users_in_schemas(&schema_names, None, None, None)
                .await?;
        let active_users =
            self.count_users_in_schemas(&schema_names, None, None, Some("active"))
                .await?;
        let new_users =
            self.count_users_in_schemas(&schema_names, Some(resolved.start), Some(resolved.end), None)
                .await?;
    ...
    pub async fn trends(
        &self,
        query: AnalyticsQuery,
    ) -> Result<AnalyticsTrendResponse, String> {
        let resolved = resolve_time_range(&query)?;
        let schema_names = self.list_schema_names().await?;
    ...
    pub async fn breakdown(
        ...
        let items = match dimension.as_str() {
            "plan_type" => self.breakdown_merchants("plan_type", resolved.start, resolved.end).await?,
            "merchant_type" => {
                self.breakdown_merchants("merchant_type", resolved.start, resolved.end).await?
            }
            _ => self.breakdown_merchants("status", resolved.start, resolved.end).await?,
        };
...
fn resolve_time_range(query: &AnalyticsQuery) -> Result<ResolvedRange, String> {
    ...
    let granularity = normalize_granularity(query.granularity.as_deref().unwrap_or(DEFAULT_GRANULARITY));
    let now = Utc::now();
...
fn parse_to_utc(raw: &str, is_end: bool, timezone: &str) -> Result<DateTime<Utc>, String> {
    ...
    let naive =
        NaiveDateTime::parse_from_str(&trimmed, "%Y-%m-%d %H:%M:%S").map_err(|_| "无效日期时间格式".to_string())?;
    let local = offset
...
fn timezone_offset(timezone: &str) -> Result<FixedOffset, String> {
    match timezone {
        ...
        "UTC" | "UTC+0" => FixedOffset::east_opt(0)
            .ok_or_else(|| "invalid timezone".to_string()),
